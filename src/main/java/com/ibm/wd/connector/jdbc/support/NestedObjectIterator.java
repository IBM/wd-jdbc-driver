...
public class NestedObjectIterator implements Iterator<Object> {
    ...
    public static NestedObjectIterator EMPTY_ITERATOR = new NestedObjectIterator(
    ...
    public NestedObjectIterator(PathCompSlice underneathSlice, String topFieldPath, Iterator<Object> source, boolean generateNestedStrictly) {
    ...
    private boolean allUnderneathHaveNext() {
        return !underneathSlice.hasNextComp() || underneathHasNext() && underneath.allUnderneathHaveNext();
    }
    ...
    private NestedObjectIterator getIteratorHoldingCurrentSequence() {
        return previousUnderneath != null ? previousUnderneath : underneath;
    }
    ...
    public Pair<String, Integer> getCurrentSequenceNumberHavingPath(String[] pathComps) {
        if (underneathSlice.relatively(pathComps) == PathCompSlice.MatchState.MAY_BRANCH_NEXT) {
            return getIteratorHoldingCurrentSequence().getCurrentSequenceNumberHavingPath(pathComps);
        }
        return new ImmutablePair<>(topFieldPath, currentSeqNum);
    }
